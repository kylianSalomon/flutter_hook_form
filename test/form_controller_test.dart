...
void main() {
  group('FormFieldsController Tests', () {
    ...
    setUp(() {
      ...
      controller = FormFieldsController<TestFormSchema>(
        formKey,
        validators: {
    ...
    test('fieldKey returns a GlobalKey for a form field', () {
      final key = controller.fieldKey(.email);
      expect(key, isA<GlobalKey<FormFieldState>>());
    });
    ...
    test('clearForcedErrors removes all forced errors', () {
      ...
      expect(
        controller.getFieldForcedError(.email),
        equals('Email error'),
      );
      expect(
